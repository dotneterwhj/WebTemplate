    ...
    where TDbContext : EFCoreContext
{
    ...
    public virtual async Task<List<TEntity>> GetPageListAsync(int skipCount, int maxResultCount, string sorting,
        bool includeDetails = false, CancellationToken cancellationToken = default)
    {
        var queryable = includeDetails ? await IncludeDetailsAsync(cancellationToken) : GetQueryable();
...
public abstract class RepositoryBase<TDbContext, TKey, TEntity> : RepositoryBase<TDbContext, TEntity>,
    ...
    where TDbContext : EFCoreContext
{
    ...
    {
        return includeDetails
            ? await (await base.IncludeDetailsAsync(cancellationToken)).FirstOrDefaultAsync(t => t.Id.Equals(id),
                cancellationToken)
